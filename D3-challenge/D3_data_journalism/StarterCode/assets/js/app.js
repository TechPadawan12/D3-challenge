...
// space placing for words
var labelArea = 110;

// bottom text padding and left axes
var tPadBot = 40;
var tPadLeft = 40;
...
// Set radius for each dot appearing on graph as a function.
...
// nest element on the bottom axes labels.
svg.append("g").attr("class", "xText");
//
var xText = d3.select(".xText");

//text is transfomed and and translated whenver the marguns change.
...
//append three text SVG files y coordinates that spaces out the values.
// 1. Poverty
xText
  .append("text")
  .attr("y", -26)
...
// nesting attr in a function making changes on window easier.
...
function visualize(theData) {
  ...
  // Grouping dots and their labels.
  var theCircles = svg.selectAll("g theCircles").data(theData).enter();

  // Append the circles for each row of data
  theCircles
    .append("circle")
    // location, size and class attributes.
    .attr("cx", function(d) {
    ...
    // rules when mouse/pointer hovers
  ...
  d3.selectAll(".aText").on("click", function() {
    ...
    if (self.classed("inactive")) {
      ...
      if (axis === "x") {
        ...
        // use a transition when axis is updated.
        svg.select(".xAxis").transition().duration(300).call(xAxis);

        // Change axis , updating location of the state circles.
